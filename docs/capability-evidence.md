...
`20260730-225411` 把同一个洞捅得更大：落地那几秒 HUD 在「剩余小队数量」和「速度」
之间抖，31 秒里 `IN_MATCH_ALIVE` 进了 5 次。

```
195.0  IN_MATCH_ALIVE
197.2  FREEFALL          ← 落地过程中 HUD 反复切
...    （四个来回）
226.8  DECISION_PAUSED CYCLE_DETECTED {state: IN_MATCH_ALIVE, entries: 5}
       wait:CYCLE_PAUSED ×1178，直到 931 秒会话结束
```

**整整 704 秒一次输入都没发**，近战停在 16 次。操作者当时的现象是「这把没近战，
把程序关掉重开就有了」——重开清掉的正是这个内存里的闩。不是键位，键位一直是对的。

这一份还暴露了第二半：闩**永远不会自己解开**。老代码的过期清理写在 `note_state`
里，而那个方法在画面没变时第一行就返回；226.8 秒之后画面一直认不出来，于是
180 秒窗口再也没被推进过，本该在 406 秒失效的记录一直留着。清理现在挪到每次
`decide` 都做，只跟时间有关。
