...
- **dxcam 的输出枚举是 import 那一刻做一次，之后永不更新。** `DXFactory.__init__`
  枚举适配器和输出，`__factory` 是模块级单例，之后每次 `create()` 都从那份快照里
  取 `Output`。所以显示拓扑一变，重建拿到的是指向已经不存在的拓扑的旧指针 ——
  **挨个输出索引探测也没用，它压根没在看现在的显示器**。这就是"假负载明明插着、
  dxcam 却从来不往那边看"的真正原因。`capture.py` 现在会在重建时丢掉那个单例、
  清掉 camera 缓存、重建 factory，等于在进程内做了一次以前只有重启才有的重新枚举。
- **假负载最省事的用法仍然是当唯一的显示输出**：「显示设置」里选
  **「仅在 <假负载> 上显示」**，并确认它能设成 **2560×1440**（便宜的假负载 EDID
  里常常只有 1080p/4K，设不出 1440p 就换一个或改 EDID），然后再关真显示器。
  一旦真显示器不在桌面上，它关不关、拔不拔都不再产生拓扑变化 —— 不是因为捕获
  扛不住，而是因为最好的拓扑变化是不发生的那次。**别在 runner 跑的时候切显示模式**，
  那本身就是一次 access loss。
