...
if (@("validate", "observe", "live", "start", "play", "account-cycle", "account-cycle-check", "ea-preflight", "probe-input") -notcontains $Command) {
    throw "Unknown command: $Command (supported: validate, observe, live, start, play, account-cycle, account-cycle-check, ea-preflight, probe-input)"
}
if ($args.Count -gt 1) {
...
elseif (@("play", "account-cycle", "ea-preflight") -contains $Command) {
